<?php
	function db_connect(){
		$conn = mysqli_connect("localhost", "root", "", "tos");
		if(!$conn){
	...
	function getBypcode($conn, $pcode){
		$query = "SELECT pname, pprice FROM product WHERE pcode = '$pcode'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function getpprice($pprice){
		$conn = db_connect();
		$query = "SELECT pprice FROM product WHERE pcode = '$pprice'";
		$result = mysqli_query($conn, $query);
		if(!$result){
	...
	function getAll($conn){
		$query = "SELECT * from product ORDER BY pcode DESC";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
